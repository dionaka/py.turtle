...
# Import VRMWidget for VRM model support
from vrm_mod.vrm_widget import VRMWidget
...
class DesktopPetApp(QMainWindow):
    ...
    def switch_model(self):
        ...
        if self.model_display_area:
            self.model_display_area.setParent(None)
            self.model_display_area.deleteLater()

        # 判断模型类型
        if next_model_path.lower().endswith('.vrm'):
            self.model_display_area = VRMWidget(next_model_path)
        else:
            self.model_display_area = Live2DWidget(initial_model_path=next_model_path)

        self.model_display_area.setAttribute(Qt.WidgetAttribute.WA_TranslucentBackground, True)
        self.model_display_area.setStyleSheet("background: transparent;")
        self.centralWidget().layout().insertWidget(0, self.model_display_area, 4)
